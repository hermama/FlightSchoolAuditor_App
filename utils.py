...
def str_to_time(timestamp,tzsource=None):
    ...
    if type(tzsource) == datetime.datetime and timestamp_dt.tzinfo == None:
        new_dt = timestamp_dt.replace(tzinfo=tzsource.tzinfo)
        return new_dt

    elif type(tzsource) == str and timestamp_dt.tzinfo == None:
        tz = pytz.timezone(tzsource)
        return tz.localize(timestamp_dt)
...
def daytime(time,daycycle):
    ...
    date_yyyy = time.strftime("%Y")
    date_mmdd = time.strftime("%m-%d")

    try:
        day_time = date_yyyy + "-" + date_mmdd + "T" + daycycle[date_yyyy][date_mmdd]['sunrise']
        night_time = date_yyyy + "-" + date_mmdd + "T" + daycycle[date_yyyy][date_mmdd]['sunset']

    except:
        return None

    if time.tzinfo == None:
        sunrise_time = str_to_time(day_time)
        sunset_time = str_to_time(night_time)

        if (time > sunrise_time) and (time < sunset_time):
            return True
        else:
            return False
    else:
        sunrise_time = str_to_time(day_time,daycycle['timezone'])
        sunset_time = str_to_time(night_time,daycycle['timezone'])
